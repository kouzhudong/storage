...
本库主要包含存储相关的内容，
如：
1.磁盘(MBR/GPT/UEFI/还原点)
2.卷（挂载点/日志记录/卷影）
3.目录（目录监控）
4.文件（流，硬链接，符号连接，稀疏文件，分页文件，元数据）
5.压缩，是压缩算法，非压缩文件，也非压缩属性。
6.文件版本，特指PE等文件。
7.哈希
8.签名，主要是PE文件等。
9.权限，仅限ACL，不包括特权及用户的信息。
10.文件夹。


特别注释：
1.文件映射不属于文件，属于内存管理（对象或句柄）的概念范畴。
2.严格区分目录和文件夹。目录对应磁盘，文件夹可以是虚拟的。
3.综上所述的功能，主要是是实现diskpart.exe和fsutil.exe两个程序的基本功能。
